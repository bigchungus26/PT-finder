...
import { currentUser, mockCourses, mockQuestions, mockGroups } from '@/data/mockData';
...
const Courses = () => {
  ...
  const getCourseStats = (courseId: string) => {
    ...
    return { questions: questions.length, groups: groups.length };
  };
        ...
        <div className="mt-12">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {mockCourses.map(course => {
              const isEnrolled = userCourses.some(uc => uc.course.id === course.id);
              const stats = getCourseStats(course.id);
              return (
                <Link
                  key={course.id}
