...
import { ShoppingBag, Minus, Plus, Trash2, Package, ArrowRight, Store } from 'lucide-react';
    ...
    return (
      <AppLayout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center">
          ...
          <Link to="/browse">
            <Button>Browse Supplements</Button>
          </Link>
        </div>
      </AppLayout>
    );
        ...
        <div className="p-4">
          ...
          <div className="space-y-3 mt-3">
            {items.map((item) => (
              ...
              >
                {/* Image */}
                <Link to={`/products/${item.product.id}`} className="w-16 h-16 rounded-xl bg-muted flex items-center justify-center overflow-hidden shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.flavor && (
                    <Badge variant="secondary" className="text-xs mt-0.5">{item.flavor}</Badge>
                  )}
          ...
          <p className="text-xs text-muted-foreground text-center mt-3">
            Payment is cash on delivery. Delivery fee added at checkout.
          </p>
        </div>
      </div>
